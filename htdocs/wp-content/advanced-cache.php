...
if (false === $plugin_location) {
	...
	if (strcasecmp('http://heinlein-wp.local/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-heinlein-wp.local.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-heinlein-wp.local.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-heinlein-wp.local.php');
}
